...
const authMiddlewareTutor = require("./authMiddlewareTutor");
const PORT = 3000;
const app = express();
...
app.use(authMiddlewareTutor);

app.post("/transits", async (req, res) => {
  if (req.body.tutorId === undefined || req.body.tutorId === null)
    return res.sendStatus(400);
  if (!req.body.plate || req.body.plate.length <= 0) return res.sendStatus(400);
  const prevTransit = await Transit.findOne({
    where: {
      tutorId: req.body.tutorId - 1,
      carPlate: req.body.plate
    }
  });
  //YYYY-MM-DDTHH:mm:ss.mmmZ
  if (prevTransit) {
    //verificare se fare multa
    const lastMoment = moment(prevTransit.createdAt);
    const hours = moment().diff(lastMoment, "hours");

    const speed = 5 / hours;
    if (speed > 130) {
      await Fine.create({
        price: Math.random() * speed * 10,
        reason: "Exceeded speed limit",
        carPlate: req.body.plate
      });
    }

    await prevTransit.destroy();
  }
